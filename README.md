...
- **Vercel**: deploy the Next.js app (set env vars in Vercel project settings).
- **Socket.IO** (optional): deploy `server/socket-server.mjs` to a Node host (Render/Railway) and set `NEXT_PUBLIC_SOCKET_URL` in Vercel to that URL.
